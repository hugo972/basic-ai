...
impl Matrix {
    ...
    pub fn add(&self, other: &Matrix) -> Matrix {
        self.apply_with(other, |val, other_val, _| val + other_val)
    }

    pub fn apply(&self, func: impl Fn(f64, usize) -> f64) -> Matrix {
        let data = self
            .data
            .iter()
            .enumerate()
            .map(|(val_index, val)| func(*val, val_index))
            .collect();
        Matrix {
            cols: self.cols,
            data,
    ...
    pub fn apply_with(&self, other: &Matrix, func: impl Fn(f64, f64, usize) -> f64) -> Matrix {
        ...
        let data = self
            ...
            .zip(other.data.iter())
            .enumerate()
            .map(|(val_index, (val, other_val))| func(*val, *other_val, val_index))
            .collect();
        Matrix {
            cols: self.cols,
            data,
